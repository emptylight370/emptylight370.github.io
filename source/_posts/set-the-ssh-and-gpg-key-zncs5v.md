...
如果要配置其他客户端的SSH免验证连接，则需要将本机的SSH公钥复制到目标客户机的authorized_keys中。现有教程是教用`ssh-copy-id`​命令进行操作，不过我没有成功过，就手动将公钥打印出来发送到目标客户机，追加到authorized_keys文件的末尾。
