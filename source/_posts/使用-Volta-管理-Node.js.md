...
小版本更新如上，而如果是大版本更新则不清楚是如何，应该还是调用安装时使用的 Node.js 版本。
...
这里输入的版本号支持多种格式，具体参考安装 npm 包时的版本号。在安装好之后会在 `package.json` ​写入具体的版本号。`pin` ​命令会将对应版本写入最近的 `package.json` ​文件。
