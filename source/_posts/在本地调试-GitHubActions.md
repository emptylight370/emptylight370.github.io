...
可以使用以下命令为 act 设置 GitHub 镜像源：

```powershell
act --github-instance=<your mirror>
```

注意这里的镜像源需要支持 clone 方式，并且不带 https 协议头。现在 act 会将 `git clone https://github.com/user/repo` ​替换成 `git clone https://<your mirror>user/repo`，如此就设置好了镜像源。实际上这个命令是用于 GHE 的，但是既然是替换掉默认的 github.com，那么也可用于配置镜像源。

具体命令的解释请在此页 [Runners - act - User Guide | Manual | Docs | Documentation](https://nektosact.com/usage/runners.html) 底部查看。个人推荐将通用的命令行参数（pull、镜像源等）直接写在 VSCode 扩展设置里面，其他参数使用方法 2 手动开关。
